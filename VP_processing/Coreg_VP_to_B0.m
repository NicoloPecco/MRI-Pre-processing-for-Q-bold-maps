function [outputArg1,outputArg2] = Coreg_VP_to_B0(pathDCE)
...
%   Detailed explanation goes here
    ...
    % Taking T1 
	PG = spm_select('FPList',[pathDCE,'/'] , '^B.*bet.nii$'); 
    if isempty(PG)
        error('No Target B0 DCE selected. Check files or file names!');
    end      
	PG = PG(1,:);
	VG = spm_vol(PG);

    % SOURCE: FLAIR 
	PF = spm_select( 'FPList', [pathDCE,'/'], '^VP.*.nii$');   
	if isempty(PF)
     	error('No Source VP!');
	end
    PF = PF(1,:);
